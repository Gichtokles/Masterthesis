...
for(i in 1:ntot){
  ...
  beta_hat_candidate <- (bigC_candidate%*%t(xc)%*%y)/sigma_draw
  
  
  aaa <- (((delta_draw^(-(2*k_candidate+2)))*det(SIGMA_candidate)^(-(1/2))*(det(bigC_candidate)^(1/2)))*exp(-(1/2)*(crossprod(y)/sigma_draw-t(beta_hat_candidate)%*%solve(bigC_candidate)%*%beta_hat_candidate)))/
